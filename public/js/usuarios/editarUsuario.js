...
async function editarUsuario(id_usuario) {
    try {
        ...
        // Espera un pequeño tiempo para asegurarte de que la modal anterior se haya eliminado
        ...
        // Mostrar la modal
        const myModal = new bootstrap.Modal(modalContainer.querySelector("#editarUsuarioModal"));
        myModal.show();
...
async function cargarDatosUsuarioEditar(id_usuario) {
    try {
        ...
        if (response.status === 200) {
            const { id, nombre, edad, cedula, sexo, telefono, cargo, avatar } = response.data;
